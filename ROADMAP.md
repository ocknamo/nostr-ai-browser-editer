...
### Phase 2: CD（継続的デプロイ）と公開

> Phase 1 完了直後に実施する。公開URLがないとブラウザからの動作確認ができず、以降のフェーズの開発が進めにくいため。
...
#### 2-2. Cloudflare Pages セットアップ

| 対象 | 作業内容 |
|------|---------|
| Cloudflare Dashboard | GitHub リポジトリを連携し Pages プロジェクトを作成 |
| ビルド設定 | Build command: `npm run build` / Output directory: `dist` / Root directory: `browser-app` |
| 環境変数 | 必要な環境変数を Cloudflare Pages の Settings → Environment Variables に設定 |

#### 2-3. GitHub Actions による CD パイプライン（任意）
...
Phase 1〜5 完了後に取り組む機能拡張候補。

### FW-1: Nostrによるログイン・設定の永続化

Nostr の秘密鍵／公開鍵ペアをアイデンティティとして利用し、ユーザー設定（リポジトリ・ブランチ・openFile など）を Nostr イベントとして保存・復元する。

| 対象 | 作業内容 |
|------|---------|
| 認証フロー | NIP-07（ブラウザ拡張: Alby, nos2x など）または秘密鍵直接入力でログイン |
| 設定の永続化 | ユーザー設定を Nostr の kind:30078（アプリ固有データ）などで暗号化・保存 |
| 復元フロー | ログイン時に Relay から設定イベントを取得してアプリ状態を復元 |
| UI | Header にログイン状態表示・ログアウトボタンを追加 |

**メリット:** アカウント不要・サーバーレスで設定をどのデバイスからでも共有できる。

### FW-2: Nostrを介したコーディング指示の送受信

ブラウザアプリから Nostr メッセージを送信し、外部のコーディングエージェント（OpenCraw 等のサービス、または Claude Code など）にコーディング指示を行う。

| 対象 | 作業内容 |
|------|---------|
| 指示送信UI | チャット入力欄を復活させ、Nostr DM（NIP-17）または NIP-90（Data Vending Machine）で指示を送信 |
| レスポンス受信 | エージェントからの返答を Relay 経由でリアルタイム受信・表示 |
| エージェント接続設定 | 接続先エージェントの npub や Relay URL を設定画面で管理 |
| StackBlitz 連携 | コーディング完了後に Preview を自動リロードして結果を確認 |

**背景・意図:**
現状 Claude Code のスマホアプリを主なターゲットとしているが、Nostr をメッセージングレイヤーとして採用することで：
- Claude Code 以外のコーディングエージェント（OpenCraw 等）への対応が可能になる
- クライアント・エージェント間の結合を疎にし、将来的な選択肢を広げる
- Nostr の分散・検閲耐性の特性により、特定プラットフォームへの依存を排除できる
