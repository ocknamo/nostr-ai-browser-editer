...
describe('parseGitHubRepo', () => {
  ...
  it('extracts only owner/repo when URL contains extra path segments', () => {
    expect(parseGitHubRepo('https://github.com/facebook/react/tree/main/packages')).toBe('facebook/react');
  });
  ...
  it('returns trimmed input for a non-GitHub URL', () => {
    expect(parseGitHubRepo('https://gitlab.com/owner/repo')).toBe('https://gitlab.com/owner/repo');
  });

  it('returns trimmed input for a plain string that is not owner/repo format', () => {
    expect(parseGitHubRepo('  some-invalid-input  ')).toBe('some-invalid-input');
  });
});
